...
| 2026-09-11 | **Инцидент: HTTPS недоступен (поправка — хостинг ни при чём).** Сайт на хостинге владельца (панель зелёная, FTP/deploys идут). Симптом: TCP 443 открыт, но TLS ClientHello уходит в тишину; при этом тот же порт мгновенно отвечает на нешифрованные запросы — слушатель жив. Почерк сетевой фильтрации TLS, а не упавшего сервера; совпало с режимом беспилотной опасности и резаньем мобильного интернета (телефон владельца не открывает ни с VPN, ни без). Тест обратного звонка ждёт нормализации сети. Сервер со стороны хостинга проверен владельцем — всё работает там; повторная проба 09:15 — снаружи по-прежнему глухо. |
