...
| 2026-09-11 | **Прод проверен по FTP: лежит новый код** (`$callbackTo` на месте). Версия со старым кодом отпала: `mail()` рапортует успех, а локальный MTA хостинга молча теряет письма по дороге к MX (sender verify / очередь). Форма при этом говорит «Спасибо» — ложный успех. Дальше: либо тикет в поддержку (проверить очередь почты), либо запланированный fallback на SMTP-авторизацию с честными кодами ошибок. |
