...
| 2026-09-11 | **Обратный звонок: получатель → служебный ящик.** По просьбе владельца заявки временно шлём на site@ (в коде помечено ТЕСТ, прод — stas@). Сквозная проверка: письмо, побайтово повторяющее сборку хендлера (From webmaster@, сырая кириллица в теме, то же тело), доставлено в служебный ящик и прочитано по IMAP, ящик почищен. Важно: kompleks-s.ru делает sender verification — конверт от несуществующего webmaster@ отбивает 550, поэтому на проде смотрим, какой конвертный отправитель подставит `mail()`; если письма не дойдут — завести webmaster@ или задать `-f` существующего ящика (либо fallback на SMTP-авторизацию site@, как уже заложено в решениях). |
