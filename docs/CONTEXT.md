...
| 2026-09-11 | **Инцидент: HTTPS сайта лежит для всех.** Сайт не открывается ни с телефона владельца (с VPN и без), ни из моей сети: TCP 443 открыт, но TLS-handshake висит до таймаута при любом SNI; HTTP 80 жив (BitNinja+Caddy, редирект на мёртвый HTTPS). Вывод — проблема на стороне хостинга, не клиентская. Тест обратного звонка заблокирован до починки. |
