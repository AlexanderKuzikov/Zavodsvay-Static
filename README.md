...
> **Статус:** pre-static PHP-версия. Object pages реализованы (пилот 6 страниц). Фильтрация по категориям на карте реализована. Целевое состояние — pure static HTML через `build.php` после готовности WebForge-генератора.
...
- Интерактивная легенда-фильтр под картой: toggle категорий, solo-click, reset, счётчик
...
> Это относится и к общей карте, и к карте на странице объекта.

### Фильтрация
> Фильтрация реализована в самой легенде.  
> Первый клик при состоянии «все включены» оставляет только выбранную категорию (`solo-click`).  
> Повторный клик по единственной активной категории возвращает показ всех категорий.

### Спутниковый режим
> ⛔ В ymaps3 официального спутникового слоя нет (подтверждено Яндексом).  
> Вопрос переключателя типов карты — на обсуждении с заказчиком.  
> Подробнее — в `CONTEXT.md`.
...
- [x] Фильтрация по категориям на карте
- [x] OG-изображения
