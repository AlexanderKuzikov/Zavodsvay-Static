...
> **Статус:** pre-static PHP-версия. Object pages — 529 страниц programmatically сгенерированы. Фильтрация по категориям на карте реализована (общая карта + страница объекта). Локальные шрифты приведены к единому неймингу и подключены через раздельные latin/cyrillic `@font-face`. SEO-разметка всех 31 статьи полностью реализована: `og_type=article`, Schema.org Article, даты публикации `2026-01-01`. Страница каталога свай ВСГ реализована полностью. Страница цен реализована, прайс-лист проаудирован. Целевое состояние — pure static HTML через `build.php` после готовности WebForge-генератора.
