...
- [x] **OG-изображения** — `og-home.jpg` + `og-home.webp` готовы к подключению
...
### Следующий слой SEO
- `partials/head-seo.php` — единый SEO-partial для всех layouts
  - Интерфейс: page-level переменные `$og_image` (дефолт: `/assets/img/og/og-home.jpg`), `$og_type` (дефолт: `website`), `$schema_type` (дефолт: `LocalBusiness`)
  - Глобальные данные компании и geo — статично внутри partial
- Open Graph + Twitter Cards + VK
- JSON-LD Schema.org (`@graph` в `<head>`): Organization + LocalBusiness + WebSite
- geo-теги Яндекса
...
- `partials/head-seo.php` — SEO/OG/Schema.org (планируется)
...
| 2026-05-14 | OG assets готовы: assets/img/og/og-home.jpg (primary, для краулеров), assets/img/og/og-home.webp (доп.) |
| 2026-05-14 | SEO-partial: partials/head-seo.php, page-level переменные $og_image/$og_type/$schema_type, дефолты из данных компании |
