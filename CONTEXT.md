...
> ⚠️ **Координаты в `data/map.json`** хранятся как `[latitude, longitude]` (бытовая нотация).  
> ymaps3 ожидает `[longitude, latitude]` (GeoJSON).  
> Перевод везде: `coordinates: [coords[1], coords[0]]`
...
| Порядок координат | `data/map.json`: `[lat, lng]`; ymaps3: `[lng, lat]` — перевод везде |
...
| 2026-05-15 | Координаты в map.json: `[lat, lng]`; ymaps3: `[lng, lat]` — перевод: `[coords[1], coords[0]]` |
