...
- **Корневые npm-скрипты** — `package.json` в корне репо, прокси к `tools/`; запуск из корня без `cd tools`
- **Страница каталога свай** — изображения VSG-1..4 подключены через `<picture srcset>` с прямыми путями к WebP-наборам
...
- [x] **Корневые npm-скрипты** — `npm run ui/media/deploy` из корня
...
package.json                ← корень: прокси-скрипты к tools/
...
# Все последующие запуски — из корня
...
### CLI (без UI)
```bash
npm run media    # из корня
# или напрямую:
cd tools && node process-media.js
```

### Когда регистрировать в media.json, а когда нет

| Случай | Подход |
|---|---|
| Изображение переиспользуется на нескольких страницах | `render_image()` — обязательно в реестр |
| Изображение управляется через UI (alt, caption, регенерация) | В реестр |
| Статичное product-фото на одной странице (каталог, карточка) | `<picture srcset>` напрямую, без реестра |

> **Правило:** если `render_image()` не нужен — не регистрируй. Реестр для переиспользуемых и управляемых медиа.
...
| 2026-05-15 | Корневой package.json с прокси-скриптами добавлен; все команды запускаются из корня: `npm run ui`, `npm run media`, `npm run deploy`. `npm install` по-прежнему нужен только в `tools/`. |
