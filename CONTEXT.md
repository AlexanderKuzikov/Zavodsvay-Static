...
5. **Image pipeline для объектов.** Структура хранения, роль generative-моделей, связь с `data/media.json`.

6. **Хранение изображений объектов.** Сейчас в `pages/map/img/{id}_N.webp`. При масштабировании — вынести в `assets/img/objects/`.

7. **Переключатель типов карты.** Схема/спутник/гибрид в ymaps3 официально недоступны в API v3. Обсудить с заказчиком.

8. **Номенклатура диаметров свай.** Диаметры в прайсе (60, 73, 102, 114 мм) не совпадают с диаметрами в каталоге (57, 76, 89, 108... мм). Ждём ответа заказчика.
