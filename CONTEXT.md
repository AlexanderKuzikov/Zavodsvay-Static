...
- **Главная страница** — вариант `/preview-d/` согласован с заказчиком и **выполнено** (2026-05-20)
...
Идея создания отдельного `data/objects.json` как расширенного SEO-реестра (с `metaTitle`, `metaDescription`, `schema` и пр.) была **отвергнута на стадии обсуждения** и никогда не реализовывалась.

**Решение:** единственный источник истины для объектов — `data/map.json`. SEO-поля для object pages генерируются программно из имеющихся данных в `_template.php`. Никакого второго реестра нет и не будет.
...
7. **Переключатель типов карты.** Схема/спутник/гибрид в ymaps3 официально недоступны. Обсудить с заказчиком.
