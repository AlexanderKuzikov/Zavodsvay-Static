...
- **Главная страница `/preview-d/`** — альтернативный вариант главной, согласован с заказчиком, находится в доработке
...
- [ ] **Главная страница** — вариант `/preview-d/` отправлен заказчику, **в доработке**
...
## Главная страница `/preview-d/` (в доработке)

### Статус
- Вариант `/preview-d/` согласован с заказчиком как основа
- Отправлен заказчику для ревью: **2026-05-20**
- Текущий этап: доработка по фидбеку заказчика

### Архитектура
- `layouts/preview-d.php` — отдельный layout с собственными стилями (`.hd-*` namespace), не наследует styles из `main.php`
- `pages/index/content.html` — пока содержит старый SEO-контент (заменить после финализации `/preview-d/`)
- CSS инлайн в layout, изолирован через `.hd-` prefix
- После финализации: перенести верстку в `pages/index/content.html` + `layouts/home.php`
...
### Известные точки доработки
| # | Проблема | Статус |
|---|---|---|
| 1 | Эмодзи в иконках (`.hd-advantage__icon`, `.hd-service__icon`) — AI-антипаттерн | ⏳ На доработке |
| 2 | `onerror` fallback на изображениях объектов (`/pages/map/img/{id}_1.webp`) — нет гарантии наличия файлов | ⏳ На доработке |
| 3 | Цифры в hero: `14 лет` (2012→2026 ✓), `600+ объектов` (в `data/map.json` — 529) — уточнить с заказчиком | ⏳ Уточнить |
| 4 | Карта в `/preview-d/` — проверить подключение Яндекс.Карт JS API v3 vs заглушка | ⏳ Проверить |

### Договорённость
- После доработки и согласования `/preview-d/` → верстка становится `pages/index/content.html` + финальный `layouts/home.php`
- Вопрос 9 из «Открытых архитектурных вопросов» считается закрытым после мержа
...
9. ~~**Главная страница.**~~ — вариант `/preview-d/` отправлен заказчику, в доработке. **Закрывается после мержа.**

10. **Номенклатура диаметров свай.** Диаметры в прайсе (60, 73, 102, 114 мм) не совпадают с диаметрами в каталоге (57, 76, 89, 108... мм). Ждём ответа заказчика — что является актуальной номенклатурой. После ответа обновить либо каталог, либо прайс.
...
### Поиск по карте (реализовано)

- Инпут `.map-search` расположен между `#map-works` и `.map-legend`
- Поиск по полям: `title` + `techDescription` через `String.includes()` (регистронезависимо)
- `debounce(300ms)` — кластеризатор не дёргается на каждый keypress
- Dropdown `.map-search__results` показывается под инпутом при наличии запроса
- Каждая строка: цветная точка категории + `title` + `techDescription` (обрезается, ellipsis)
- Объекты с `url` — тег `<a>`, без `url` — `<div>` без hover
- Max 15 строк; если результатов больше — footer «Ещё N объектов — уточните запрос»
- Пустой результат — «Ничего не найдено»
- Кнопка `×` — очистить запрос и закрыть dropdown
- Закрытие: `Escape`, клик вне `.map-search`, очистка инпута
- **`applyFilters()`** — единая функция фильтрации: объединяет `activeCategories` + `searchQuery`, обновляет `clusterer`, счётчик, карточки под картой
- При активном поиске `renderCards()` получает все опубликованные объекты из результатов (без «по одному из категории»)
- Смена категории при активном поиске → dropdown перефильтровывается с учётом нового состояния
...
- `layouts/preview-d.php` — экспериментальная главная страница (`.hd-*` namespace)
...
| 2026-05-18 | Страница цен: `pages/prices/content.html` содержал полный DOCTYPE-документ — убран, оставлен только контент-фрагмент |
| 2026-05-18 | Страница цен: аудит прайса — выявлены расхождения диаметров с каталогом, возможная опечатка в Ø114/4000/ВСГ-3, вопросы по нагрузкам. Запрос отправлен заказчику, ждём ответа. |
| 2026-05-18 | Поиск по карте: `title` + `techDescription`, `debounce(300ms)`, dropdown max 15 строк, кнопка очистки `×`, закрытие по Escape/клик вне/очистка |
...
| 2026-05-20 | Главная страница: вариант `/preview-d/` (`layouts/preview-d.php`, namespace `.hd-*`) отправлен заказчику, находится в доработке |
| 2026-05-20 | `/preview-d/` структура: Hero+видео, Аудитория, Преимущества (6), Услуги (4), Сваи vs лента/плита, Статистика, Примеры работ (6 объектов), CTA |
| 2026-05-20 | Известные точки доработки preview-d: эмодзи→иконки, fallback изображений объектов, уточнение цифр (14 лет / 600+ объектов), проверка карты |
